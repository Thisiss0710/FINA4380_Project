                    ...
                    if x == 0:
                        ...
                        for y in range(len(pca_array)+1):  # y is the yt1h beta of stock n 
                            if y == 0:
                                ...
                                single_cov = beta_expected_matrix[m,x]*beta_expected_matrix[n,y]*pca_cov.loc[x-1,y-1]
                                whole_cov = whole_cov + single_cov
                ...
                for x in range(len(pca_array)+1):
                    for y in range(len(pca_array)+1):                
                        if x == 0 and y == 0:
                            ...
                            single_cov = pca_array[y-1] * beta_matrix[x,y]
                            whole_cov = whole_cov + single_cov
                        else:
                            single_cov = beta_expected_matrix[m,x]*beta_expected_matrix[m,y]*pca_cov.loc[x-1,y-1] + pca_array[x-1]*pca_array[y-1]*beta_matrix[x,y] + pca_cov.loc[x-1,y-1]*beta_matrix[x,y] 
                            whole_cov = whole_cov + single_cov
